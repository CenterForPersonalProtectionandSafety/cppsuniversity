  ...
  <div class="col-sm-12">
    ...
    <form class="form-signup" action="<?=$form_action;?>" method="<?=$form_method;?>">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" id="username" name="username" placeholder="User Name" value="<?php if (!$form_valid && !empty($_POST)){ echo $username;} ?>" required autofocus autocomplete="username"><?php } ?>


        <label for="fname">First Name*</label>
        <input type="text" class="form-control" id="fname" name="fname" placeholder="First Name" value="<?php if (!$form_valid && !empty($_POST)){ echo $fname;} ?>" required autofocus autocomplete="first-name">
        ...
        $character_range = "min"." ".$settings->min_pw . " ". "and" ." ". $settings->max_pw . " " . "max" ." ". "char";
        ...
        <div style="display: inline-block">
          <label for="password">Password* (min <?=$settings->min_pw?> and max <?=$settings->max_pw?> char)</label>
          <input  class="form-control" type="password" name="password" id="password" placeholder="Password" required autocomplete="password" aria-describedby="passwordhelp">

          <label for="confirm">Confirm Password*</label>
          <input  type="password" id="confirm" name="confirm" class="form-control" placeholder="Confirm Password" required autocomplete="password" >
        </div>
        <div style="display: inline-block; padding-left: 20px">
          <strong>Password Should...</strong><br>
          ...
          <br><br>
          <a class="nounderline" id="password_view_control"><span class="fa fa-eye"></span> Show Password</a>
        </div>
        <br><br>
        ...
        <?php if($settings->show_tos == 1){ ?>
          ...
          <label><input type="checkbox" id="agreement_checkbox" name="agreement_checkbox"> I Accept User Terms and Conditions</label>
        <?php } //if TOS enabled ?>
      </div>
